...
def _fetch_url(url: str, headers: dict = None, timeout: int = 10):
    if requests:
        return requests.get(url, headers=headers or {}, timeout=timeout).text
...
async def search_ransomware_leaks(query: str):
    ...
    try:
        raw_text = await asyncio.to_thread(_fetch_url, url)
