...
def _check_platform_urllib(platform: dict, username: str) -> Optional[dict]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=5) as resp:
            ...
            if not_found_text:
                ...
                if not_found_text.lower() in body.lower():
                    return None
            if status in (200, 301, 302, 307, 308):
                return {"platform": platform["name"], "url": url, "status": status, "cat": platform.get("cat", "misc")}
    except Exception:
        pass
    return None
...
async def phantom_id_search(username: str):
    ...
    if not results and aiohttp is None:
        loop = asyncio.get_running_loop()
        # Sample high priority platforms with urllib
        tasks = [loop.run_in_executor(None, _check_platform_urllib, p, username) for p in PLATFORMS[:20]]
        responses = await asyncio.gather(*tasks, return_exceptions=True)
        for r in responses:
            if r and isinstance(r, dict):
                results.append(r)
    ...
    threat_val = int(min(98, max(5, risk_score * 2.5 if found > 0 else 0)))
    ...
    return {
        ...
        "threat_level": "CRITICAL" if threat_val >= 70 else ("ELEVATED" if threat_val >= 35 else "NOMINAL"),
