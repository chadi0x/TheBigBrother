...
def _calculate_mse(diff_img) -> float:
    try:
        pix = list(diff_img.getdata())
        if not pix:
            return 0.0
        total_sq = sum(((p[0]**2 + p[1]**2 + p[2]**2) / 3.0) for p in pix)
        return round(total_sq / len(pix), 4)
    except Exception:
        return 0.0


def _color_channel_stats(image: Image.Image) -> dict:
    try:
        rgb = image.convert("RGB")
        bands = rgb.split()
        r_vals = list(bands[0].getdata())
        g_vals = list(bands[1].getdata())
        b_vals = list(bands[2].getdata())
        total = len(r_vals) or 1
        r_avg = round(sum(r_vals) / total, 2)
        g_avg = round(sum(g_vals) / total, 2)
        b_avg = round(sum(b_vals) / total, 2)
        sample = r_vals[:1200]
        r_var = round(sum((x - r_avg)**2 for x in sample) / len(sample), 2) if sample else 0.0
        g_sample = g_vals[:1200]
        g_var = round(sum((x - g_avg)**2 for x in g_sample) / len(g_sample), 2) if g_sample else 0.0
        b_sample = b_vals[:1200]
        b_var = round(sum((x - b_avg)**2 for x in b_sample) / len(b_sample), 2) if b_sample else 0.0
        return {
            "red_mean": r_avg,
            "green_mean": g_avg,
            "blue_mean": b_avg,
            "red_variance": r_var,
            "green_variance": g_var,
            "blue_variance": b_var,
            "dominant_spectrum": "Red" if r_avg >= max(g_avg, b_avg) else ("Green" if g_avg >= b_avg else "Blue")
        }
    except Exception:
        return {}
...
def _ela_multi(image: Image.Image) -> dict:
    ...
    # Measure average ELA intensity and exact MSE at each quality level
    intensities = {}
    mse_scores = {}
    for q, label, _ in [(95, "q95", 15), (75, "q75", 20), (50, "q50", 25)]:
        try:
            ...
            mse_scores[label] = _calculate_mse(diff)
        except Exception:
            intensities[label] = None
            mse_scores[label] = 0.0
    ...
    results["description"] = "ELA at 3 quality levels with MSE calculations. Bright = high error. Inconsistent bright zones across levels = edits."
    return results
...
def _clone_detection(image: Image.Image, block_size: int = 32) -> dict:
    ...
    try:
        ...
        return {
            ...
            "interpretation": (
                f"[ALERT] {len(clone_pairs)} cloned region pairs detected — likely background cloning or object removal" if len(clone_pairs) > 2
...
def _detect_ai_artifacts(image: Image.Image) -> dict:
    ...
    try:
        ...
        if not exif:
            ...
            for tool in _AI_SOFTWARE:
                if tool in software:
                    signals.append(f"[FLAGGED] AI software tag confirmed: '{software[:120]}'")
                    score += 80
                    break
    ...
    verdict = (
        "[CRITICAL] CONFIRMED AI-GENERATED" if score >= 80 else
        "[ELEVATED] VERY LIKELY AI-GENERATED" if score >= 55 else
        "[SUSPICIOUS] POSSIBLE AI ARTIFACTS" if score >= 30 else
        "[NOMINAL] LIKELY AUTHENTIC" if score >= 12 else
        "[AUTHENTIC] VERIFIED OPTICAL RESIDUALS"
    )
                ...
                if tag == "GPSInfo":
                    ...
                    try:
                        ...
                        if "GPSLatitude" in gps_raw and "GPSLongitude" in gps_raw:
                            ...
                            meta["warnings"].append("[ALERT] GPS LOCATION EMBEDDED — Image exposes physical location coordinates")
                    except Exception:
                        pass
                    ...
                    meta["exif"][tag] = str(value)
                elif tag in ("ISOSpeedRatings", "ExposureTime", "FNumber", "FocalLength"):
                    if "exposure" not in meta:
                        meta["exposure"] = {}
                    meta["exposure"][tag] = str(value)
                    meta["exif"][tag] = str(value)
                else:
                    sv = str(value)
                    if len(sv) < 400:
...
def pixel_forge_analyze(file_bytes: bytes, filename: str) -> dict:
    ...
    """
    if Image is None:
        return {"status": "error", "error": "PIL/Pillow engine not installed in Python environment", "success": False, "verdict": "UNAVAILABLE"}

    try:
        image = Image.open(io.BytesIO(file_bytes))
        image.load()
    except Exception as e:
        return {"status": "error", "error": f"Could not open image: {e}"}
    ...
    color_stats = _color_channel_stats(image)
    ...
    overall_verdict = (
        ...
        "[LOW_RISK] SOME ANOMALIES"         if composite_risk >= 20 else
        "[NOMINAL] CLEAN — NO SIGNIFICANT ARTIFACTS"
    )
